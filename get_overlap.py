...
command="blastn -query contigs_500-Passengers_S0_L001.fasta -db db/contigs_500-Sailors_S2_L002 -num_threads 20 -outfmt 6 -max_target_seqs 1 -perc_identity 98 -max_hsps 1"
...
with open("contigs_500-Undetermined_S0_L001.fasta", "r") as original, open("contigs_500-Undetermined_S0_L001_common.fasta", "w") as output:
    sequences = SeqIO.parse(original, "fasta")
    for sequence in sequences:
        if sequence.id in seq_names:
            SeqIO.write(sequence, output, "fasta")

command="blastn -query contigs_500-Passengers_S0_L001_common.fasta -db db/nt -num_threads 60 -outfmt 6 -max_target_seqs 1 -max_hsps 1"
result = subprocess.run(command,shell=True, capture_output=True, text=True)
